	...
	...) 
{
	...
	p <- ggplot(data=.df, aes(group=strata, colour=strata)) +
	    #geom_step(aes(x=time, y=surv), size = lWidth) +
	    geom_step(aes(linetype = strata, x=time, y=surv), size = lWidth) +
	    theme_bw() +
			...
			scale_x_continuous(xlabs, breaks = times, limits = xlims) +
			scale_y_continuous(ylabs, limits = ylims) +
			opts(legend.position = c(xStrata,yStrata)) +    # MOVE LEGEND HERE [first is x dim, second is y dim]
			opts(legend.key = theme_rect(colour = NA)) +
			#labs(linetype = ystrataname) +
			opts(plot.margin = unit(c(0, 2, .5,ifelse(m < 10, 2.5, 2.5)),"lines")) +
			opts(title = main)
	...
	if(pval) 
	{
		...
		pvaltxt <- ifelse(pval >1,"p < 0.0001",paste("pv =", signif(pval, 3)))
		#p <- p + annotate("text",x = 0.6 * max(sfit$time),y = 0.1,label = pvaltxt)
		p <- p + annotate("text",x = xPV * max(sfit$time),y = yPV,label = pvaltxt)
	}
...
myData = read.csv("Sample_table.csv", sep=",")
fit <- survfit(Surv(time,status)~group, data=myData)


ggkm_new(fit, timeby=12, xlabs = "Month")
